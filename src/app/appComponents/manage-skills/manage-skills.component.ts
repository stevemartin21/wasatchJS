...
export class ManageSkillsComponent implements OnInit {
  ...
  addPaginators() {
    if (this.skills.length % this.itemsPerPage === 0) {
      this.numberOfPaginators = Math.floor(this.skills.length / this.itemsPerPage);
    } else {
      this.numberOfPaginators = Math.floor(this.skills.length / this.itemsPerPage + 1);
    }
